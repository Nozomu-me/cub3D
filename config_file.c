...
/*   Updated: 2020/03/14 14:16:00 by amouassi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_sub_config_file(t_infos *infos, char *line)
{
	if (ft_start_with(line, "R"))
		...
		infos->c = ft_parsing_colors(line, "C", infos);
	else if (ft_start_with(line, "F"))
		infos->f = ft_parsing_colors(line, "F", infos);
}
...
t_infos		ft_config_file(char *str)
{
	...
	{
		ft_print_error("Error\nconfig file structure is invalid", &infos);
	}
	return (infos);
}
